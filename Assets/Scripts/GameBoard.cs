...
public class GameBoard : MonoBehaviour
{
    ...
    public GameObject GameOverText,GameMaster;
...
public void SelectTrioAround(Vector2 touchPoint)//dokunma pozisyonu ve 3lü secim
{
   ...
   public void TurnCW()//saat yonu tersinde donus
   {
       if (!selectedTiles.Contains(null))
       {
    ...
    public void TurnCCW()//saat yonunde donus
    {
        if (!selectedTiles.Contains(null))
        {
            Vector2 center = new Vector2((selectedTiles[0].Item3.transform.position.x + selectedTiles[1].Item3.transform.position.x + selectedTiles[2].Item3.transform.position.x) / 3, (selectedTiles[0].Item3.transform.position.y + selectedTiles[1].Item3.transform.position.y + selectedTiles[2].Item3.transform.position.y) / 3);
   ...
   public void RotationStopped()//donmesinin durması
   {
   ...
   public void Allocate(int w, int h)//destroy olanların yerine yenisi
   {
       tiles = new GameObject[w, h];
       tilesReadyToHexplode = new GameObject[w, h];
   }

    public GameObject GetTile(int i, int j)//sahne olusturulmasında kullanıldı
    {
    ...
    public void SetTile(int i, int j, GameObject o)//tekrar bak
    {
        if (i < 0 || j < 0 || i > tiles.GetLength(0) - 1 || j > tiles.GetLength(1) - 1)
        {
            return;
    ...
    private void SetMemory(int i, int j, GameObject o)//tekrar bak
    {
    ...
    public void EndInitialGenerationPhase()//tekrar tekrar sahne olusturulmasın sahne bir defa olussun methodu
    {
    ...
    private bool AllTilesAreStationary()//6genlerin sabit durması
    {
    ...
    private void MoveDownAirborneHexes() {//patlayan 6genlerin asagı dusmesi ve yerlerine yenilerinin olusması
        for (int i = 0; i < tiles.GetLength(0); ++i)
        {
            for (int j = 0; j < tiles.GetLength(1); ++j)
            {
    ...
    private void CreateNewHexesForEmptyCells()//bos yerlere yeni 6genler olusması
    {
        for (int i = 0; i < tiles.GetLength(0); ++i)
        {
            for (int j = 0; j < tiles.GetLength(1); ++j)
    ...
    private void RotateSelectedTilesInGrandArray()//secme ve dondurme olarak dusunduk denenecek
    {
    ...
    private void MarkHexplosions()//renk olarak dusunuldu denenecek
    {
        for (int i = 0; i < tiles.GetLength(0); ++i)
        {
            for (int j = 0; j < tiles.GetLength(1); ++j)
